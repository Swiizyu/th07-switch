...
#ifdef __SWITCH__
#include "../platform/switch/SwitchAudio.hpp"
#endif
...
#ifdef __SWITCH__
    // AUDOUT is hardwired to 48 kHz. Running the engine at 44100 made SDL
    // resample every buffer, on top of everything else that was going wrong.
    engineConfig.sampleRate = 48000;
    // Horizon has no miniaudio backend: run the engine device-less and pump it
    // from an SDL3 audio stream instead (see platform/switch/SwitchAudio.cpp).
...
#ifdef __SWITCH__
    if (!SwitchAudio::Start(this->engine))
    {
        g_GameErrorContext.Log("DirectSound オブジェクトの初期化が失敗したよ\n");
        SAFE_DELETE(this->engine);
        return ZUN_ERROR;
