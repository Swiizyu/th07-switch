...
ScoreDat *ResultScreen::OpenScore(const char *path)
{
    ...
    // th07-switch: ScoreDat is a POD with no constructor, so a plain `new`
    // leaves decodedData holding whatever was on the heap. On Windows/Linux
    // fresh pages happen to be zero and the "no score.dat yet" path survives;
    // on Horizon the heap is dirty and ReleaseScoreDat() then free()s a garbage
    // pointer -> Data Abort on the very first launch. Value-initialise instead.
    ...
    if (g_LastFileSize < sizeof(ScoreDatRaw))
    {
        ...
        // th07-switch: RECREATE_SCORE deletes scoreData again -> double delete.
        delete scoreData;
        scoreData = NULL;
        goto RECREATE_SCORE;
    }
