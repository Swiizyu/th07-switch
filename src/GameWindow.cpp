...
#ifdef __SWITCH__
    // th07-switch: eglSwapBuffers on Horizon/Mesa does not actually block, and
    // SDL_GL_GetSwapInterval() still reports 1, so CheckVSync() concludes there
    // is hardware vsync and disables the software limiter. The result is the
    // renderer free-running at ~120 FPS (the fps counter is telling the truth)
    // while the logic stays locked at 60, burning battery and starving the
    // audio thread. Pace presentation ourselves against an absolute deadline so
    // we neither drift nor accumulate error.
    (void)timeToRender;
    {
        static u64 nextFrameNs = 0;
        u64 now = SDL_GetTicksNS();

        if (nextFrameNs == 0 || now > nextFrameNs + nsPerFrame * 4)
        {
            // first frame, or we fell far behind (loading, HOME menu): resync
            nextFrameNs = now + nsPerFrame;
        }
        else
        {
            if (now < nextFrameNs)
