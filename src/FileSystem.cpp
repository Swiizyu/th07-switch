...
std::string FileSystem::GetBasePath(const char *filename)
{
    ...
    // On Horizon the game data lives on the SD card next to the NRO (or in
    // sdmc:/switch/th07). PlatformSwitch::Init() resolved it at startup.
    return PlatformSwitch::DataPath() + filename;
#endif
...
{
#if defined(__SWITCH__)
    // th07.cfg, score.dat, replays and snapshots are written back to the same
    // SD folder the data was loaded from.
    return PlatformSwitch::DataPath() + filename;
