...
ZunResult Supervisor::LoadConfig(const char *configFilename)
{
...
#ifdef __SWITCH__
    // One-time migration, tracked by a marker in an otherwise unused config
    // byte. Applied once per th07.cfg so the options screen stays in charge
    // afterwards: forcing these values on every load would silently undo
    // whatever the player picked in the menu.
    constexpr u8 kSwitchCfgMarker = 0x53; // 'S'
    if (g_Supervisor.cfg.unused_27[0] != kSwitchCfgMarker)
    {
    ...
    // Keep BGM in RAM: streaming it off the SD card is what made the audio
    // thread stall, and the console has memory to spare for one track.
    g_Supervisor.cfg.preloadBgm = 1;
#endif
